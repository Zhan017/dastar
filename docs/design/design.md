...
| D5 | Query layer | Kysely with plain SQL migrations and generated types | Drizzle | Constraints and triggers are hand-written SQL anyway |
...
2. Transaction: set actor (`key:<id>` or `token:<reservation_id>`). `SELECT ... FOR UPDATE` the reservation. For the token path, compare sha256.
3. `UPDATE reservation SET status = 'confirmed', confirm_token_hash = NULL WHERE id = $1 AND ($ev IS NULL OR version = $ev)`. Triggers adjudicate: DA001 if expired, DA005 if not held, version bump, audit. Zero rows is version_conflict.
